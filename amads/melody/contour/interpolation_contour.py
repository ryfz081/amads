...
def calculate_interpolation_contour(
    times: list[float], pitches: list[int]
) -> list[int]:
...
def interpolation_contour_mean_gradient(interpolation_contour: list[float]) -> float:
...
def interpolation_contour_direction_changes(
    ...
    total_changes = np.sum(interpolation_contour[:-1] != interpolation_contour[1:])
